...
class  CombinedNet(nn.Module):
	def __init__(self, num_classes, num_clusters, branch_layer, dataset):
		...
		self.mt_weights_binary = nn.ParameterList([nn.Parameter(torch.tensor(0.)) for _ in range(num_classes['binary'])])
		self.mt_weights_continuous = nn.ParameterList([nn.Parameter(torch.tensor(0.)) for _ in range(num_classes['continuous'])])
		self.mt_weights_discrete = nn.ParameterList([nn.Parameter(torch.tensor(0.)) for _ in range(num_classes['discrete'])])
	...
	def forward(self, aerial_image, ground_image):
		...
		for attr_type in ["binary", "discrete", "continuous"]:
			for i in range(self.num_classes[attr_type]):
				...
				if attr_type == 'binary':
					alpha = torch.sigmoid(self.mt_weights_binary[i])
					combined_features_weighted = alpha*ground_pool_features + (1-alpha)*aerial_pool_features
					out_binary.append(self.task_binary[i](combined_features_weighted))
				elif attr_type == 'continuous':
					alpha = torch.sigmoid(self.mt_weights_continuous[i])
					combined_features_weighted = alpha*ground_pool_features + (1-alpha)*aerial_pool_features
					out_continuous.append(self.task_continuous[i](combined_features_weighted))
				else:
					alpha = torch.sigmoid(self.mt_weights_discrete[i])
					combined_features_weighted = alpha*ground_pool_features + (1-alpha)*aerial_pool_features
					out_discrete.append(self.task_discrete[i](combined_features_weighted))
